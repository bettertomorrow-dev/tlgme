...
func (app application) runPrompt(ctx context.Context, token string, chatID any, message outgoing) error {
	...
	ans, err := app.awaitAnswer(ctx, token, numericChatID, msgID, message.buttons, sentAt, message.retries)
	if err != nil {
		...
		if errors.Is(err, errPromptTimeout) {
			if _, sendErr := app.send(ctx, token, chatID, outgoing{text: timeoutText, retries: message.retries}); sendErr != nil {
...
func awaitAnswer(ctx context.Context, token string, chatID int64, questionMsgID int, buttons []string, after time.Time, retries int) (promptAnswer, error) {
	...
	for {
		...
		case <-timer.C:
			...
			mu.Unlock()
			if !sent {
				id, sendErr := sendOutgoing(ctx, token, chatID, outgoing{text: checkinText, retries: retries})
				if sendErr == nil {
					mu.Lock()
