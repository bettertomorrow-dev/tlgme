...
func parseCLI(args []string) (cliOptions, error) {
	...
	if opts.version {
		if opts.text.set || opts.image.set || opts.file.set || opts.filename.set || opts.prompt || opts.silent || len(opts.buttons) > 0 || opts.learn || opts.token.set || opts.chatID.set || opts.setToken.set || opts.setChatID.set || opts.update || opts.dryRun {
			return cliOptions{}, errors.New("--version cannot be combined with other options")
		}
		return opts, nil
	}
	if opts.update {
		if opts.text.set || opts.image.set || opts.file.set || opts.filename.set || opts.prompt || opts.silent || len(opts.buttons) > 0 || opts.learn || opts.token.set || opts.chatID.set || opts.setToken.set || opts.setChatID.set || opts.dryRun {
	...
	if setMode {
		if opts.text.set || opts.image.set || opts.file.set || opts.filename.set || opts.prompt || opts.silent || len(opts.buttons) > 0 || opts.learn || opts.token.set || opts.chatID.set || opts.dryRun {
