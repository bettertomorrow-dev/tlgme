...
This sends the question, waits up to 5 minutes for a plain-text reply or an inline button tap, and prints the answer on stdout (button label or reply text). Use `--timeout DURATION` to set a different initial wait, for example `--timeout 90s`. Durations use Go syntax such as `30s`, `2m`, or `1h`; the default is `5m` and the value must be positive. A button tap edits the question message to append `Answer: <label>` and reacts with 👍, so the choice stays visible; a text reply gets a 👀 reaction and the buttons are removed instead. For waits longer than 30 seconds, the bot sends a check-in 30 seconds before the deadline. Shorter waits skip it. If the user reacts to a check-in, the wait extends by 5 more minutes. On timeout the command exits non-zero, the chat gets a timeout notice, buttons are removed, and stdout is empty. Treat that like any other missing answer: tell the user in chat and don't retry in a loop. Full details: [tlgme README](https://github.com/bettertomorrow-dev/tlgme#readme).
